...
- 计划与进度深化 (增量6): 从模板阶段层级派生主/子/单机计划并 FS 串联, 主子约束冲突定位, 节点重排保留已批准基线, 项目级阶段权重版本化覆盖, 实际开始/完成日期反馈, 以计划工作日为单位的挣值 (PV/EV/AC/SPI/CPI/EAC/预测完工) 与日快照趋势, 每日 06:00 本地定时扫描生成逾期提醒进入我的待办 (只写本地不投递外部), Gate 检查项例外放行需说明.
