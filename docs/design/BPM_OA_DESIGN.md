...
> **📌 已汇总**:本文档的规格与设计决策已提炼汇总至 [design.org](design.org)(单一设计文档);本文档保留原始细节与逐日 changelog,两者不一致时以 design.org 为准.
...
## 表单设计器与 Vue 版本(@form-create)差距记录

> 记录时间 2026-08-26.核心闭环已通(设计→保存→发起→回显→审批→权限),
> 以下为**体验差距**,逐步补齐.

| 差距项 | Vue 版本 | 现状 | 计划 |
|---|---|---|---|
| 拖拽添加/排序 | 原生拖拽 | 点击添加+按钮排序 | ✅ 本轮 |
| 栅格布局(多列) | Row/Col | 单列 | ✅ 本轮 |
| 事件配置(on-change) | 支持 | 无 | 待做 |
| 规则校验完整(type/len/range) | 完整 | 必填+正则 | 待做 |
| 表单配置(标签宽度/按钮) | 完整 | labelWidth 100 | 待做 |
| 子表单/富文本/地区组件 | 支持 | 无 | 待评估 |
| 模板/复制表单 | 支持 | 无 | 待做 |

- **2026-08-26 (63)** **界面浅色修复**:
  · 流程设计器强制浅色(bpm-flow-wrap 白色背景,dark 模式也保持浅色画布)
  · 侧边栏从深色(#172033)改为浅色(白色):Sider/Menu 改 light 主题,logo/文字色调整
  · sys_config sidebarTheme 改 theme-light
  · 实测:侧边栏 rgb(255,255,255),设计器画布 rgb(250,250,250);349测试0失败;E2E 5通过

- **2026-08-26 (62)** **待办详情 + 报销表单完善 + 工作菜单 icon**:
  · 待办详情:待办行加"详情"按钮 → 抽屉(流程模型/申请表单回显/流程图追踪/审批历史),复用 bpm-flow-designer 只读
  · 报销表单完善(作为完善例子):报销事由(必填min)/金额/类型(select+事件)/费用明细(子表单)/出差地区(联动type=travel)
    /发票照片(upload-image)/备注;reimburseApproval 关联 form_id=2;实测选差旅显示地区,事件填备注
  · 工作菜单 icon:DB 菜单 icon 更新为 icon-picker 可识别的 antd 图标名(23个办公菜单全配),
    icon-picker 扩展 9 个缺失图标;迁移 202608270002-bpm-menu-icons(sqlite+mysql)
  · 实测:办公栏目所有菜单 icon 渲染;349测试0失败;E2E 6通过

- **2026-08-26 (61)** **设计器差距收官:地区级联 + 表单模板 + 充分测试**:
  · 地区选择组件:cascader + 内置常用省市区数据(12省+主要市)
  · 表单模板:设计器头部"模板"下拉(请假/报销/通用申请)一键填充画布字段
  · 修复 full-demo/frontend-bpm-report E2E:受控输入竞态 → pressSequentially
  · 充分测试:SQLite+MySQL 349 测试 0 失败;前端 8 个 E2E 全过(含完整流程演示20截图)
  · 设计器差距全部清零(拖拽/栅格/校验/事件/表单配置/富文本/复制/子表单/地区/模板)

- **2026-08-26 (60)** **设计器差距(四):子表单(subform)动态行**:
  · 组件库 +子表单:设计器属性面板"子字段管理"(标题/字段名/类型/增删)
  · form_render 渲染动态行(子字段 input/number/date/select),添加/删除行,值=行数组
  · 实测:费用明细添加行填"交通费",再添一行;0 报错

- **2026-08-26 (59)** **设计器体验差距(三):表单配置 + 复制表单 + 富文本**:
  · 表单配置:设计器头部加 标签宽度/布局(纵向/横向),conf.form 持久化,发起页应用布局
  · 复制表单:流程表单列表行加"复制"按钮(详情+新建副本)
  · 富文本:自写轻量组件(contenteditable+execCommand,B/I/U/列表/链接/段落/清除),
    兼容 React 19(react-quill 2.0 与 React19 不兼容已弃用)
  · 实测:布局配置生效;复制成功;富文本输入保存

- **2026-08-26 (58)** **设计器体验差距补齐(二):校验完整 + 事件配置**:
  · 校验规则补 min/max(长度):设计器属性面板(最小/最大长度),form_render rules + bpm_start 手动校验
  · 事件配置:字段 props.on-change={set-field,set-value},变更时自动设置目标字段值;设计器属性面板"变更时设置字段"
  · 实测:事由填1字被min拦截;选"出差"自动填充摘要"已选类型"
  · 差距记录更新:拖拽/栅格/校验(min,max)/事件 已补

- **2026-08-26 (57)** **设计器体验差距补齐:拖拽 + 栅格布局**:
  · 拖拽:组件库 item 可拖入画布(HTML5 drag/drop);画布字段卡片可拖拽排序(:move 操作)
  · 栅格布局:字段 props.col-span(24/12/8/16),form_render 用 antd Row/Col 布局,属性面板加"占列宽"配置
  · 实测:拖入 3→4 卡片,拖拽排序生效;字段一/二 col-span=12 并排,整行字段下一行
  · 差距记录:拖拽/栅格已补;事件配置/完整校验/子表单/富文本/模板 待做(见设计器差距 section)
