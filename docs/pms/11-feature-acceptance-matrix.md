...
| Q | [治理测试](../../test/clj/com/ruoyi/pms_governance_test.clj): 本轮 SQLite 29 tests / 343 assertions 通过 (在 A08 任命书与 H02 干系人/RACI/沟通计划基础上再新增 C05 文档批量下载, C04 密级/阶段/结构节点归集与最新版本归集视图, C06 文档独立发布审批不漂移, B05/C07 会议会前资料绑定, H01 章程初始预算校验与版本不可变, H08 风险超阈值自动升级并独立确认解除缓解门控, C10 典型风险库实例化并复用 H08 升级门控, H02 沟通节奏标记已沟通推进下次日期与逾期预警, C09 问题逾期与阻断级读模型预警各 1 例); MySQL 本轮未执行(本地无实例), 迁移双库文件已同步; 含真实 JWT 路由, 独立审批, 文本证据, CSV, 问题重开, 风险复审与升级, 成员任命书, 干系人识别到沟通计划生成会议闭环, 文档密级归集与会前资料版本引用, 章程可选初始预算金额/币种规范化 |
...
| H02 | 工程基线新增 | 干系人, RACI与沟通计划 | 识别利益相关者及职责, 明确知会/参与/批准关系, 沟通节奏可执行并有调整记录 | B2+B3 | implemented / local | Q: stakeholder-raci-conflict-and-comm-plan-loop,stakeholder-comm-plan-http-contract,comm-plan-log-advances-next-date-and-flags-overdue; 干系人登记+不可变修订(编号不可改), RACI按活动至多一个A且不得重复指派并输出缺A/缺R冲突, 沟通计划受控修订+由最新版本生成会议并回写last_meeting_id形成闭环; 沟通节奏现可执行: "标记已沟通"记录一次实际沟通并按计划频率(日/周/双周/月/季)自动顺延下次日期且逐次追加可审计communication_log, 读模型输出comm_overdue/comm_days_until供台账到期预警; 经复用通用治理存储pms_gov_record(stakeholder/raci/comm-plan)与工作台"干系人与沟通"页签; 浏览器端到端(pms-h02.spec.js登记干系人+RACI缺A冲突+生成会议回写, pms-h02c.spec.js过期"沟通已到期"->标记已沟通按周频顺延2026-09-22+7=09-29->徽标转"N天后沟通"); 待: MySQL本轮未执行(本地无实例), 外部通知/消息渠道自动提醒与按节奏定时派发未接通 |
