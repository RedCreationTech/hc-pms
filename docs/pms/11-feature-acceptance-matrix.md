...
| Q | [治理测试](../../test/clj/com/ruoyi/pms_governance_test.clj): 本轮 SQLite 32 tests / 361 assertions 通过 (在 A08 任命书与 H02 干系人/RACI/沟通计划基础上再新增 C05 文档批量下载, C04 密级/阶段/结构节点归集与最新版本归集视图, C06 文档独立发布审批不漂移, B05/C07 会议会前资料绑定, H01 章程初始预算校验与版本不可变, H08 风险超阈值自动升级并独立确认解除缓解门控, C10 典型风险库实例化并复用 H08 升级门控, H02 沟通节奏标记已沟通推进下次日期与逾期预警, C09 问题逾期与阻断级读模型预警, 以及三项只读治理洞察读模型 各 1 例: 干系人权力-利益象限与未绑定责任人, RACI执行R职责负载与过载预警, 会议行动闭环计数与逾期集中); MySQL 本轮未执行(本地无实例), 迁移双库文件已同步; 含真实 JWT 路由, 独立审批, 文本证据, CSV, 问题重开, 风险复审与升级, 成员任命书, 干系人识别到沟通计划生成会议闭环, 文档密级归集与会前资料版本引用, 章程可选初始预算金额/币种规范化, 干系人管理策略象限, RACI职责负载与会议行动闭环只读洞察 |
...
本轮 C10/H02/C09 及三项只读治理洞察增量后全新 CLI SQLite 集成运行 `clojure -M:test -d test/clj -r 'com.ruoyi.pms.*-test'` 已通过 84 tests / 692 assertions, 0 failures/errors (本地无 MySQL 实例, 本轮新增治理用例的 MySQL 回归未执行, 迁移双库文件已同步). 此前提交 `cfe4b15` 的 [CI运行35682432061](https://github.com/RedCreationTech/hc-pms/actions/runs/35682432061) 中 SQLite和MySQL后端均已通过当时全量PMS回归. 本地8个浏览器用例, 生产构建及独立启动也已通过; Linux CI浏览器8例5.1分钟首次通过,无重试/flaky,三项作业全success,最终记录见V. 本轮另在隔离 `:3100` 后端(独立空库全新迁移)跑通 H02 治理浏览器用例 `pms-h02.spec.js`(1 passed, 无未捕获JS错误), 覆盖界面登记干系人, RACI缺A冲突提示与沟通计划生成会议回写; 并在同一隔离后端一次批跑通过新增三条浏览器用例 `pms-c10.spec.js`(典型风险库选用即触发 H08 升级门控与"风险库"来源标记), `pms-c09b.spec.js`(问题台账"已逾期/阻断级"读模型预警)与 `pms-h02c.spec.js`(沟通计划"标记已沟通"推进下次日期并翻转逾期徽标), 3 passed, 无未捕获JS错误; 又批跑通过 `pms-h02d.spec.js`(界面登记不同权力-利益组合干系人 -> "管理策略"列派生重点管理/保持满意/保持知会/持续监控四象限且未选责任人的显示"未绑定责任人"), `pms-h02e.spec.js`(同一干系人承担多条执行R -> "R职责负载"列计数并在阈值3触发"职责过载", 负责A不计入负载)与 `pms-b05b.spec.js`(会议派生多条行动 -> "行动闭环"列显示未完成 open/total 与逾期计数, 行动转真实任务后从未完成扣除), 再 3 passed, 无未捕获JS错误. 双库及浏览器通过不等于生产/UAT或所有复合条目已完成.
