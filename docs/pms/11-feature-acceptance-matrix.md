...
| Q | [治理测试](../../test/clj/com/ruoyi/pms_governance_test.clj): 本轮 SQLite 36 tests / 390 assertions 通过 (在 A08 任命书与 H02 干系人/RACI/沟通计划基础上再新增 C05 文档批量下载, C04 密级/阶段/结构节点归集与最新版本归集视图, C06 文档独立发布审批不漂移, B05/C07 会议会前资料绑定, H01 章程初始预算校验与版本不可变, H08 风险超阈值自动升级并独立确认解除缓解门控, C10 典型风险库实例化并复用 H08 升级门控, H02 沟通节奏标记已沟通推进下次日期与逾期预警, C09 问题逾期与阻断级读模型预警, 三项只读治理洞察读模型, 以及 H18 需求/文档/干系人受控软作废与受控恢复 各 1 例: requirement-discard-is-soft-and-reference-guarded, document-discard-rejects-referenced-and-non-discardable-status, stakeholder-discard-rejects-referenced-record, discarded-records-can-be-restored-with-audit); MySQL 本轮未执行(本地无实例), 迁移双库文件已同步 (含新增状态 CHECK 表重建迁移 202609220011); 含真实 JWT 路由, 独立审批, 文本证据, CSV, 问题重开, 风险复审与升级, 成员任命书, 干系人识别到沟通计划生成会议闭环, 文档密级归集与会前资料版本引用, 章程可选初始预算金额/币种规范化, 干系人管理策略象限, RACI职责负载与会议行动闭环只读洞察, 受控作废引用守卫与状态门控, 受控恢复回作废前状态与审计 |
...
| H18 | 工程基线新增 | 草稿清理与资料保留策略 | 不把取消当删除, 草稿清理需权限/引用校验, 正式历史按保留策略归档, 删除/恢复行为可审计 | B5+B7 | partial | Q; T: requirement-discard-is-soft-and-reference-guarded, document-discard-rejects-referenced-and-non-discardable-status, stakeholder-discard-rejects-referenced-record, discarded-records-can-be-restored-with-audit; SQLite 36 tests/390 assertions + 浏览器 pms-h18.spec.js 2 passed 与 pms-h18b.spec.js 2 passed; 已实现子集: 需求/文档/干系人受控软作废(状态迁移非物理删除, 保留内容与版本链)+写权限门控(403)+未知字段校验(400)+陈旧版本/状态门控(409)+引用守卫(被追踪/会议会前资料/问题风险证据/行动/RACI/沟通受众引用则 409)+作废人/原因/时间与 workflow_history 审计留痕+对称的受控恢复(仅 discarded 可恢复到作废前状态, 记 restore 审计, 免迁移)+界面"已作废"/"恢复"/"作废"入口翻转. 待: 正式历史按保留策略归档, 级联影响预览, 把已作废文档从归集口径剔除(当前 document_collection 不区分状态), MySQL 迁移回归, 生产验收 |
