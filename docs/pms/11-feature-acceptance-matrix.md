...
| Q | [治理测试](../../test/clj/com/ruoyi/pms_governance_test.clj): 本轮 SQLite 35 tests / 378 assertions 通过 (在 A08 任命书与 H02 干系人/RACI/沟通计划基础上再新增 C05 文档批量下载, C04 密级/阶段/结构节点归集与最新版本归集视图, C06 文档独立发布审批不漂移, B05/C07 会议会前资料绑定, H01 章程初始预算校验与版本不可变, H08 风险超阈值自动升级并独立确认解除缓解门控, C10 典型风险库实例化并复用 H08 升级门控, H02 沟通节奏标记已沟通推进下次日期与逾期预警, C09 问题逾期与阻断级读模型预警, 三项只读治理洞察读模型, 以及 H18 需求/文档/干系人受控软作废 各 1 例: requirement-discard-is-soft-and-reference-guarded, document-discard-rejects-referenced-and-non-discardable-status, stakeholder-discard-rejects-referenced-record); MySQL 本轮未执行(本地无实例), 迁移双库文件已同步 (含新增状态 CHECK 表重建迁移 202609220011); 含真实 JWT 路由, 独立审批, 文本证据, CSV, 问题重开, 风险复审与升级, 成员任命书, 干系人识别到沟通计划生成会议闭环, 文档密级归集与会前资料版本引用, 章程可选初始预算金额/币种规范化, 干系人管理策略象限, RACI职责负载与会议行动闭环只读洞察, 受控作废引用守卫与状态门控 |
