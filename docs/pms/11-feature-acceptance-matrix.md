...
| ID | PPT 页码 | 功能 | 业务闭环验收 | 批次 | 当前状态 | 验证证据 |
|---|---|---|---|---|---|---|
| C01 | 4,16 | URS条目手建/列表/分类 | 项目, 类别, 编号, 内容及必需/期望可维护, 项目内唯一, 修改形成版本 | B3 | implemented / local | Q: evidence-is-real-immutable-and-scoped,requirement-verification-method-is-optional-enum-persisted; B: pms-c02v.spec.js; 手工登记和不可变URS修订, 具体字段见治理合同; 需求可声明可选验证方式枚举(test/inspection/demonstration/analysis, 免迁移随 payload 持久化, `s/enum!` 校验非法取值 400, 未选中下拉的空串/缺键视为未设定零回归, 台账"验证方式"列 geekblue 标签可视, 修订可改值而原版本不漂移, 五列 CSV 导入不受影响)已 implemented/local; 诚实边界: 验证方式目前仅为登记属性, 尚未与追踪矩阵"验证需求"关系或关闭证据做联动校验, 也未做按验证方式聚合的只读统计 |
