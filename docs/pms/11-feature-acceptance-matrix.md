...
| C06 | 4,58 | 文档交付与审批归档 | 从编制到评审/发布/签发留版本轨迹, Gate引用不可变版本, 新版本不漂移旧决定 | B2+B3 | partial | Q: gate-uses-pinned-evidence-and-formal-waiver,document-release-requires-independent-approval-and-does-not-drift,document-release-coverage-is-derived-read-only; B: pms-c06.spec.js(界面登记文档->提交发布选独立审核人->审核人批准发布->发布状态"已发布", 新修订回到"已登记"且旧批准版本不漂移), pms-c06b.spec.js(界面登记4文档+独立审核人批准/驳回->面板显示"覆盖文档4/已发布率25%/待审1/未提交1/已驳回1", UI对已批准文档发新版本后覆盖文档仍4且已发布率降到0%证明按 code 最新有效版本聚合修订不重复计数, HTTP作废已驳回版本后覆盖文档降到3且已驳回归零证明剔除已作废最新版本); 已登记不可变证据并锁定Gate引用, 新增文档独立发布审批链(registered/rejected->in_review->approved/rejected, 复用reviewer!/decision-actor!职责分离, 提交与决定走latest!只能在最新版本推进, 批准记released_by, 新修订不漂移旧批准); 本轮增补按每个文档编号最新有效版本只读聚合的"证据发布覆盖度"(免迁移, 免新命令, 免新 kind): 读模型新增派生字段 release_coverage, 由 governance.evidence/release-coverage 统计 {total, approved, in-review, registered, rejected, released-pct}(以 s/latest 按 code 取最高 revision 为口径故修订不重复计数, 剔除最新版本已作废 discarded, 已发布率整数化), 前端"证据版本"页签新增"证据发布覆盖度"面板(覆盖文档数蓝标+已发布率绿/金/红+各状态计数标签); 诚实边界: 覆盖度只反映发布审批链所处状态, 不等于内容合规或已挂 Gate; 正式电子签章, 外部文书模板与覆盖度-Gate引用联动仍待实现 |
| C07 | 4,17-18 | 会议管理与会后行动追踪 | 会前资料/参会人员/纪要, 行动项负责人和期限, 提醒/完成/转计划闭环 | B3 | partial | Q: meeting-action-creates-one-real-task,meeting-action-completion-verifies-independently-and-flags-overdue,meeting-can-reference-real-document-versions-as-pre-read-materials,meeting-action-closure-counts-and-overdue,owner-workload-aggregates-open-items-across-kinds,issue-and-action-due-countdown-flags-remaining-days; B: pms-c07.spec.js(逾期标记->提交完成附证据与独立审批人->核验人独立关闭),pms-b05.spec.js(界面选择证据文档版本作为会前资料->读模型回显 material_ids->台账会前资料列计数->引用不存在/跨项目/非文档版本经真实HTTP 404, 未知字段400),pms-b05b.spec.js(界面登记会议并逐条形成行动->"行动闭环"列显示未完成 open/total 与逾期计数->行动转真实任务后从未完成计数扣除),pms-g-load.spec.js(同一责任人 admin 界面登记 2 问题+1 风险并 HTTP 挂 1 条会议行动 -> "责任人负载"列在"风险与问题"与"会议行动"两张台账一致回显跨类"未关闭 x 4"与红色"负载过重" -> 界面"转为WBS任务"后三台账同步降到"未关闭 x 3"且"负载过重"消失, 真实HTTP GET governance 回显三台账 admin 行 owner_open_load=3),pms-due.spec.js(HTTP 挂一条约+2天到期会议行动->行动台账"到期倒计时"列显示"剩 N 天临期"->界面"转为WBS任务"后倒计时列不再显示临期/逾期且真实HTTP回显 action_due_in_days=null/action_due_soon=false); 纪要/参与人/责任期限/任务来源/完成证据/独立核验/逾期计算已实现, 会前资料现可绑定项目内真实不可变文档版本(0..50不重复, 非法404/400, 留空[]), 会议台账新增只读"行动闭环"洞察(行动总数/未完成/逾期集中, 转真实任务不计入未完成), 并新增跨问题/风险/行动三台账的只读"责任人负载"洞察(owner_open_load 统计同一责任人当前未关闭事项总数, 排除 closed/converted, 达阈值4 提示"负载过重", 用于暴露一人被跨类集中指派的负载失衡), 会议行动台账新增只读"到期倒计时"洞察(action_due_in_days 剩余天数负值即逾期, action_due_soon 1..3 天临期, 已关闭或已转真实任务不计)均为读取时派生不落库不投递; 自动到期提醒仍待补齐 |
...
| C09 | 4,59 | 问题分类分级及处理关闭 | 提出人/责任人/严重度/目标日明确, 处理证据经验证再关闭, 超期可跟踪 | B3 | partial | Q: risk-becomes-one-issue-and-requires-independent-verification,closed-issue-reopens-only-through-independent-review,issue-reassign-changes-owner-with-audit-and-guards-membership,issue-read-model-flags-overdue-and-blocker,issue-escalation-requires-independent-acknowledgment-before-resolution,issue-and-action-due-countdown-flags-remaining-days; B: pms-c09.spec.js(界面转派->新责任人+原因留痕, 非成员/缺原因/越权/关闭后均被拒),pms-c09b.spec.js(登记到期日已过且严重度阻断的问题->台账"逾期预警"列同时出现"阻断级"与"已逾期"; 远期一般问题不预警; 读模型issue_overdue/issue_critical二次确认),pms-c09d.spec.js(界面登记阻断级问题->"超阈值升级"列显示"待升级确认/management", 登记人看不到确认入口且提交解决被真实HTTP 409门控/自确认403, 独立审批人"确认升级处置"后徽标翻转"升级已确认"且提交解决放行200; 逾期阻断级升级到steering, major不触发升级可径直提交解决),pms-due.spec.js(界面登记约+2/+30/-5天到期问题->问题台账"到期倒计时"列分别显示"剩 N 天临期"/"剩 N 天"/"已逾期 N 天", 真实HTTP回显issue_due_soon/issue_due_in_days与issue_overdue同口径),pms-ri.spec.js(由"风险发生,转问题"生成的问题在问题台账"来源风险"列以标签回显来源风险标题, 手工登记问题标"手工登记"; HTTP回显 issue_source_risk_id/title); 已有分级/期限/证据/验证/受控重开/责任人转派, 超期与阻断的只读预警标记已实现(按服务器当天计算, 非提醒投递), 阻断级问题登记即自动升级并须独立质量审批人确认后方可提交解决的闭环已实现; "到期倒计时"只读列把二元逾期细化为剩余天数/临期/逾期分档(读取时派生不落库不投递); 由风险materialize生成的问题暂不自动升级, 逾期后追溯升级与提醒投递仍待实现 |
