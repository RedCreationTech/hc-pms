...
| C04 | 4,58 | 按阶段/结构归集文档 | 立项/设计/DQ/制造/验证/过程文档分层管理, 文档编号/版本/状态/创建人/密级可追踪 | B3 | partial | Q: evidence-is-real-immutable-and-scoped,document-classification-stage-and-structure-are-traceable; B: pms-c04.spec.js(界面登记密级/阶段->台账显示密级与阶段列, 缺省内部, 非法密级400, 修订编号不可改); 实际UTF-8文本版本/SHA256/创建人可追踪, 新增密级(public/internal/confidential)/阶段/结构节点作为不可变归集字段随版本持久化并进入批量下载清单; 尚无按阶段/节点多层归集视图, 密级过滤与二进制存储仍待实现 |
