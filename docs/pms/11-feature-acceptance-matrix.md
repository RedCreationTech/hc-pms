...
| Q | [治理测试](../../test/clj/com/ruoyi/pms_governance_test.clj): 本轮 SQLite 38 tests / 414 assertions 通过 (在 A08 任命书与 H02 干系人/RACI/沟通计划基础上再新增 C05 文档批量下载, C04 密级/阶段/结构节点归集与最新版本归集视图, C06 文档独立发布审批不漂移, B05/C07 会议会前资料绑定, H01 章程初始预算校验与版本不可变, H08 风险超阈值自动升级并独立确认解除缓解门控, C10 典型风险库实例化并复用 H08 升级门控, H02 沟通节奏标记已沟通推进下次日期与逾期预警, C09 问题逾期与阻断级读模型预警, 三项只读治理洞察读模型, 以及 H18 需求/文档/干系人受控软作废与受控恢复, H18c 已作废最新版本从归集剔除与级联影响只读预览 各若干例: requirement-discard-is-soft-and-reference-guarded, document-discard-rejects-referenced-and-non-discardable-status, stakeholder-discard-rejects-referenced-record, discarded-records-can-be-restored-with-audit, document-collection-excludes-discarded-latest-versions, discard-preview-reports-guards-without-mutating); MySQL 本轮未执行(本地无实例), 迁移双库文件已同步 (含新增状态 CHECK 表重建迁移 202609220011); 含真实 JWT 路由, 独立审批, 文本证据, CSV, 问题重开, 风险复审与升级, 成员任命书, 干系人识别到沟通计划生成会议闭环, 文档密级归集与会前资料版本引用, 章程可选初始预算金额/币种规范化, 干系人管理策略象限, RACI职责负载与会议行动闭环只读洞察, 受控作废引用守卫与状态门控, 受控恢复回作废前状态与审计, 级联影响预览与归集剔除已作废 |
...
| C09 | 4,59 | 问题分类分级及处理关闭 | 提出人/责任人/严重度/目标日明确, 处理证据经验证再关闭, 超期可跟踪 | B3 | partial | Q: risk-becomes-one-issue-and-requires-independent-verification,closed-issue-reopens-only-through-independent-review,issue-reassign-changes-owner-with-audit-and-guards-membership,issue-read-model-flags-overdue-and-blocker; B: pms-c09.spec.js(界面转派->新责任人+原因留痕, 非成员/缺原因/越权/关闭后均被拒),pms-c09b.spec.js(登记到期日已过且严重度阻断的问题->台账"逾期预警"列同时出现"阻断级"与"已逾期"; 远期一般问题不预警; 读模型issue_overdue/issue_critical二次确认); 已有分级/期限/证据/验证/受控重开/责任人转派, 超期与阻断的只读预警标记已实现(按服务器当天计算, 非提醒投递), 问题自动升级规则仍待实现 |
