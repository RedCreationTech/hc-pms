...
- `verified`: 下述精确子能力有实现及通过记录. 范围为 Batch 1 的SQLite/MySQL后端验证及SQLite环境的Chrome全链路, 不外推为生产或全模块验收.
...
| 证据 | 实际记录及范围 |
|---|---|
| V | [Batch 1 验证记录](verification.md): SQLite/MySQL后端各10 tests / 75 assertions, 浏览器2 passed, SQLite迁移往返及构建通过, CI三个作业均通过; 生产/外部系统未验收 |
...
| H19 | 工程基线强化 | 非功能与运行就绪 | MySQL目标环境, 迁移升级/回退, 压测, 监控告警, 备份恢复与RPO/RTO演练通过 | B7 | planned | 待: 生产目标环境升级/回退/部署/容量/恢复报告; 已有CI MySQL新库及B1测试不能替代生产演练 |
