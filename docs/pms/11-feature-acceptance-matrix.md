...
| H08 | 工程基线强化 | 风险和问题的复审/升级/重开 | 风险评分口径和复审频率明确, 超阈值升级, 问题有验证人/关闭证据且可受控重开 | B3 | partial | Q: closed-issue-reopens-only-through-independent-review,risk-review-requires-evidence-and-future-followup,risk-escalation-requires-independent-acknowledgment-before-mitigation,risk-review-due-countdown-flags-remaining-days,risk-review-rescore-recomputes-escalation-gate; B: pms-h08.spec.js,pms-rrc.spec.js,pms-rs.spec.js; 独立重开/复审/证据关闭已有, 评分达到阈值的重大风险新建时自动升级并由登记人之外独立质量审批人确认方可解除缓解门控已 implemented/local, 风险台账"到期倒计时"只读洞察(以 `review_due_date` 复审到期日回落 `due_date` 为基准, 免迁移读取时派生剩余天数/临期/逾期, 与既有 `review_overdue` 同口径不投递)已 implemented/local, 复评期间成对提议新概率与影响以重新评分并由独立审批人批准后重算 `score` 与重新判定同一套超阈值升级门控(达阈值重新 `escalated`/`pending`, 未达阈值清理 escalation 键, 拒绝或关闭维持原评分并清理 `review_proposed_*` 提议键; 评分与升级判定纯函数抽取到 `risk-assessment` 命名空间供登记/库实例化/复评重算共用, 免迁移)已 implemented/local; 升级通知投递, 跨项目风险汇总升级仍待实现 |
