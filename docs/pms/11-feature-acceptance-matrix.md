...
| Q | [治理测试](../../test/clj/com/ruoyi/pms_governance_test.clj): 本轮 SQLite 39 tests / 437 assertions 通过 (在 A08 任命书与 H02 干系人/RACI/沟通计划基础上再新增 C05 文档批量下载, C04 密级/阶段/结构节点归集与最新版本归集视图, C06 文档独立发布审批不漂移, B05/C07 会议会前资料绑定, H01 章程初始预算校验与版本不可变, H08 风险超阈值自动升级并独立确认解除缓解门控, C10 典型风险库实例化并复用 H08 升级门控, H02 沟通节奏标记已沟通推进下次日期与逾期预警, C09 问题逾期与阻断级读模型预警, C09d 问题阻断级登记即自动升级并须独立确认方可提交解决, 三项只读治理洞察读模型, 以及 H18 需求/文档/干系人受控软作废与受控恢复, H18c 已作废最新版本从归集剔除与级联影响只读预览 各若干例: requirement-discard-is-soft-and-reference-guarded, document-discard-rejects-referenced-and-non-discardable-status, stakeholder-discard-rejects-referenced-record, discarded-records-can-be-restored-with-audit, document-collection-excludes-discarded-latest-versions, discard-preview-reports-guards-without-mutating, issue-escalation-requires-independent-acknowledgment-before-resolution); MySQL 本轮未执行(本地无实例), 迁移双库文件已同步 (含新增状态 CHECK 表重建迁移 202609220011); 含真实 JWT 路由, 独立审批, 文本证据, CSV, 问题重开, 风险复审与升级, 问题阻断级自动升级门控, 成员任命书, 干系人识别到沟通计划生成会议闭环, 文档密级归集与会前资料版本引用, 章程可选初始预算金额/币种规范化, 干系人管理策略象限, RACI职责负载与会议行动闭环只读洞察, 受控作废引用守卫与状态门控, 受控恢复回作废前状态与审计, 级联影响预览与归集剔除已作废 |
...
| C07 | 4,17-18 | 会议管理与会后行动追踪 | 会前资料/参会人员/纪要, 行动项负责人和期限, 提醒/完成/转计划闭环 | B3 | partial | Q: meeting-action-creates-one-real-task,meeting-action-completion-verifies-independently-and-flags-overdue,meeting-can-reference-real-document-versions-as-pre-read-materials,meeting-action-closure-counts-and-overdue; B: pms-c07.spec.js(逾期标记->提交完成附证据与独立审批人->核验人独立关闭),pms-b05.spec.js(界面选择证据文档版本作为会前资料->读模型回显 material_ids->台账会前资料列计数->引用不存在/跨项目/非文档版本经真实HTTP 404, 未知字段400),pms-b05b.spec.js(界面登记会议并逐条形成行动->"行动闭环"列显示未完成 open/total 与逾期计数->行动转真实任务后从未完成计数扣除); 纪要/参与人/责任期限/任务来源/完成证据/独立核验/逾期计算已实现, 会前资料现可绑定项目内真实不可变文档版本(0..50不重复, 非法404/400, 留空[]), 会议台账新增只读"行动闭环"洞察(行动总数/未完成/逾期集中, 转真实任务不计入未完成), 自动到期提醒仍待补齐 |
