...
> **所有文档与代码中的标点必须使用英文半角标点(ASCII punctuation).**
> 全角标点(,.;:()""!?等)禁止出现在任何文档和代码注释中;
> 正文语言不限(中文,英文均可),仅标点必须半角化.

适用范围:Markdown / Org 文档,PlantUML 源码,代码注释与 docstring,
commit message,日志输出.注意:代码字符串字面量若为用户可见 UI 文案
(如 antd 提示语),其标点属于产品文案,不在此规范强制范围内,修改前需
确认.新增和修改内容必须遵守;既有文档在下次触碰时顺手规范化.
...
| [docs/design/BPM_OA_DESIGN.md](./docs/design/BPM_OA_DESIGN.md) | BPM/OA 顶层设计原文 + 63 条逐日 changelog(已汇总至 design.org) |
