-- 增量6 计划与进度深化 (MySQL): 治理记录新增 stage-weights / reschedule / progress-snapshot / reminder 四类 (建新表-拷贝-改名重建 CHECK);
-- 计划任务与执行反馈增加实际开始/完成日期; 登记每日进度扫描定时任务.
...
--;;
INSERT IGNORE INTO sys_job (job_id, job_name, job_group, invoke_target, cron_expression, misfire_policy, concurrent, status, remark) VALUES (9001, 'PMS进度扫描', 'PMS', 'com.ruoyi.task/pms-progress-scan', '0 0 6 * * ?', '3', '1', '0', '每日 06:00 生成各执行中项目的进度快照 (PV/EV/AC 趋势) 与本地逾期提醒, 只写本地待办不投递外部消息');
--;;
