...
(defn- dep-graph [data selected on-select]
  (let [{:keys [order dependencies]} data
...
(defn- trace-log-line [log]
  [:div {:style {:fontFamily "monospace"
                 ...
                 :padding "4px 0"}}
   [:span {:style {:color (case (:type log)
                            "call" "#1890ff"
                            "return" "#52c41a"
                            "error"  "#ff4d4f"
                            "#999")}}
    (str "[" (:type log) "]")]
   " "
   [:span {:style {:color "#666"}} (pr-str (:value log))]])
        ...
        [:div
         ...
         [:div {:style {:display "grid"
          ...
          [antd/card {:title "静态配置 (Config)"
                      :size "small"
                      :bodyStyle {:padding 12 :maxHeight 480 :overflow "auto"}}
          ...
          [antd/card {:title (str "运行时详情: " (or selected "-"))
           ...
           [antd/descriptions {:bordered true :size "small" :column 1}
            ...
            [antd/descriptions-item {:label "被依赖"}
             (if (seq dents)
               ...
               "-")]
            (when is-fn?
              [antd/descriptions-item {:label "调用追踪"}
               [antd/switch {:checked (:active trace-data)
                             :checkedChildren "开启"
           ...
           (when (and is-fn? (seq (:logs trace-data)))
             [:div {:style {:marginTop 12}}
              [:div {:style {:fontWeight 500 :marginBottom 8}} "调用日志"]
         ...
         [:div {:style {:marginTop 16}}
          [antd/card {:title "依赖图 (Dependency Graph)"
           ...
           [dep-graph data selected #(set-selected! %)]]]]))))
